...
class Encoder(nn.Module):
    ...
    def forward(self, x, atq, attn_mask=None):
        # x [B, L, D]
        attns = []
        if self.conv_layers is not None:
            for attn_layer, conv_layer in zip(self.attn_layers, self.conv_layers):
                x, attn = attn_layer(x, atq, attn_mask=attn_mask)
                x = conv_layer(x)
                attns.append(attn)
...
class Model(nn.Module):
    ...
    def forecast(self, x_enc, cycle_index, x_mark_enc=None, x_dec=None, x_mark_dec=None):
        ### ablation for ATQ
        gather_index = (cycle_index.view(-1, 1) + torch.arange(self.d_model, device=cycle_index.device).view(1, -1)) % self.cycle_len
        query_input = self.adaptiveTemporalQuery[gather_index].permute(0, 2, 1)  # (b, s, c)
        ...
        enc_out, attns = self.encoder(enc_out, query_input, attn_mask=None)
