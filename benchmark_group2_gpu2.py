...
if __name__ == '__main__':
    # --- 根据用户输入设置测试模式 ---
    # --- 设定为完整测试模式 ---
    print("--- Running in BATCH mode for Group 2 on GPU 2 ---")
    E_LAYERS_LIST = [1, 2, 3]
    ...
    # 修正后：在 CPU 和 GPU 2 上进行测试
    devices_to_test = [torch.device('cpu')]
    if torch.cuda.is_available():
        devices_to_test.append(torch.device('cuda:2'))
    ...
    # +++ 修改下面这行代码 +++
    output_filename = "results/benchmark_results_group2.csv"
